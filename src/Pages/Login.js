...
export default function Login() {
  ...
  const login = async () => {
    try {
      const responseRecieve = await axios.get(
        `http://127.0.0.1:5000/get_user_data?user_name=${loginData.userName}`
      );
  ...
  return (
    <div className="login-container">
      <form className="login-form">
        ...
        <a className="forgot-password-link" onClick={handlePassword}>
          Forgot Password
        </a>
      </form>
      {/* {message && <p>{message}</p>} */}
